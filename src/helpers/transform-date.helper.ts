...
import { DateFormat, monthArray } from '../config/index';
	...
	switch (type) {
		...
		case 'll':
			return lang === 'en'
				...
				: /* lang === 'zh_CN' */
				  `${year} 年${month ? `${monthArray[lang][month - 1]}` : ''}${
						day ? ` ${day} 日` : ''
				  }`;
	}
};
...
export const mapDate = {
	...
	zh_CN: {
		L: (date: string) => transformDate({ date, lang: 'zh_CN', type: 'L' }),
		l: (date: string) => transformDate({ date, lang: 'zh_CN', type: 'l' }),
		ll: (date: string) => transformDate({ date, lang: 'zh_CN', type: 'll' }),
	},
};
